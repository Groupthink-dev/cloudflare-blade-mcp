...
A token-efficient MCP server for the Cloudflare platform. 31 tools across DNS management, Workers KV, D1 databases, and Cloudflare Tunnels — with safety gates on every destructive operation.

## Why another Cloudflare MCP?

| | Official CF MCP | mcp-cloudflare | **This** |
|---|---|---|---|
| **Scope** | Workers/KV/R2/D1 (no DNS) | DNS + Workers + KV | DNS + KV + D1 + Tunnels |
| **Tools** | 12 | 14 | 31 (targeted) |
...
- **Broad platform coverage** — DNS, KV, D1, and Tunnels from a single server.
...
You can scope the token to only the domains you need. DNS-only users don't need account-level permissions.
...
├── server.ts                — MCP server factory, registers all 31 tools
